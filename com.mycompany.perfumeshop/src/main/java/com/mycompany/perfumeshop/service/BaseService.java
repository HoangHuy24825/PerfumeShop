...
public abstract class BaseService<E extends BaseEntity> {
	...
	public E getBySeo(String seo) {
		...
		return (E) entityManager.createNativeQuery("SELECT * FROM " + tbl.name()+" WHERE seo='"+seo+"'", clazz()).getResultList().get(0);
	}
	
	@SuppressWarnings("unchecked")
	public List<E> findAll() {
		Table tbl = clazz().getAnnotation(Table.class);
		return entityManager.createNativeQuery("SELECT * FROM " + tbl.name()+" ORDER BY updated_date DESC,created_date DESC", clazz()).getResultList();
	}

	@SuppressWarnings("unchecked")
	public List<E> findAllActive() {
		Table tbl = clazz().getAnnotation(Table.class);
		return entityManager.createNativeQuery("SELECT * FROM " + tbl.name()+" WHERE status=1 ORDER BY updated_date DESC,created_date DESC", clazz()).getResultList();
	}
	...
	public List<E> findByPage(Integer currentPage, Integer pageSize) {
		try {
			currentPage--;
			Table tbl = clazz().getAnnotation(Table.class);
			Query query = entityManager.createNativeQuery("SELECT * FROM " + tbl.name()+" ORDER BY updated_date DESC,created_date DESC", clazz());
