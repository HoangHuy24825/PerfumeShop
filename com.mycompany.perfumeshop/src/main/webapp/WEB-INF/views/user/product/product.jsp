                            ...
                            <div class="widgets_inner" style="padding-right:5px;padding-left:5px">
                            	<table class="table-categoty" id="table-category">
					                <tr style="height:50px;" class="rounded-5 row-left-sidebar" id="">
					                    <td width="50px">
					                        <img src="${base }/user/img/logo_all.png" style="width: 20px; height: 20px; padding: 2px;" />
					                    </td>
					                    <td>Tất cả các sản phẩm</td>
					                  <%--   <td><span>(${totalProduct })</span></td> --%>
					                </tr>
					                <c:forEach var="category" items="${categories }">
					                	 <tr style="height:50px;" class="rounded-5 row-left-sidebar" id="${category.id }">
					                        <td width="50px">
					                        	<img src="${base }/upload/${category.avatar}" alt="${category.name}" style="width: 20px;height: 20px;" />
					                        </td>
					                        <td>${category.name}</td>
					                       <%--  <td><span>(${category.products.size() })</span></td> --%>
					                    </tr>
					                </c:forEach>
					            </table>
                            </div>
    	...
    	$.ajax({
    		...
    		success: function(result) {
    			...
    			if ((result.products.length == 0 || result.products == null) && searchStr != null && searchStr != "") {
    				...
    				html += ' <img src="${base}/user/img/notFoundProduct.png" width="300" style="margin:auto"/>';
    				html += '<br/>'
    				...
    				$.each(result.products, function(index, item) {
    					...
    					html += '<input type="hidden" id="view_'+item.id+'" name="custId" value="'+item.seo+'">';
